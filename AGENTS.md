...
- Server Component가 아닌 곳에서 인라인 Server Action을 선언할 때, 지시어 문자열을 **반드시 `"use server"`로 정확히 작성**합니다. (`"useServer"` 등의 오타 주의)

  ```typescript
  // ✅ 올바른 사용
  <form action={async () => { "use server"; await someAction(); }}>

  // ❌ 잘못된 사용 (런타임 에러 발생)
  <form action={async () => { "useServer"; await someAction(); }}>
  ```
...
### [5] Dynamic Route 파라미터 (Next.js 15+ 필수 규칙)
Next.js 15 이상에서 Dynamic Route의 `params`는 **비동기(Promise) 객체**입니다. 반드시 아래 패턴을 따라야 합니다.
...
// ❌ 잘못된 사용 (타입 에러 및 빌드 실패)
export default async function CoursePage({
  params,
}: {
  params: { courseId: string }; // Promise가 아니면 빌드 실패
}) {
  const { courseId } = params; // await 누락
  // ...
}
```

### [6] useSearchParams() Suspense 래핑 (빌드 에러 예방)
클라이언트 컴포넌트에서 `useSearchParams()`를 사용할 때 해당 컴포넌트를 **반드시 `<Suspense>`로 래핑**해야 합니다. 누락 시 정적 프리렌더 단계에서 빌드가 실패합니다.

```typescript
// ✅ 올바른 패턴: useSearchParams를 사용하는 내부 컴포넌트를 분리하고 Suspense로 감싼다
...
| 백엔드 스킬 지침 (Prisma/DB) | `.agents/skills/backend/SKILL.md` (추후 생성 예정) |
| 프론트엔드 스킬 지침 (React/TS/UI) | `.agents/skills/frontend/SKILL.md` (추후 생성 예정) |
| 작업 계획·승인·완료 보고 절차 | `docs/ai/workflow.md` (추후 생성 예정) |
